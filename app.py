...
# Streamlit 레이아웃 설정
st.title("별과 행성의 공통질량 중심 궤도 운동과 파장 변화 시뮬레이션")
...
# 공통질량 중심에서의 비율에 따라 위치 계산
common_center_distance = (planet_mass * distance) / (star_mass + planet_mass)

# 사용자 입력을 통한 행성의 각도 설정
...
# 별의 좌표는 공통질량 중심 반대 방향으로 이동
star_x = -common_center_distance * np.cos(angle_rad)
star_y = -common_center_distance * np.sin(angle_rad)
...
ax1.legend()
ax1.set_title("별과 행성의 공통질량 중심 궤도 운동")
st.pyplot(fig1)
